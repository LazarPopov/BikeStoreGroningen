...
export async function generateMetadata({
  ...
  const description = isDutch
    ? `Bekijk fietsreparatie, tweedehands fietsen en lokale fietshulp van ${businessName} per buurt en bekende plek in ${siteConfig.city}.`
...
export default async function NeighborhoodsOverviewPage({ params }: PageProps) {
  ...
  const residentialPages = getResidentialNeighborhoodPagesForSite(siteConfig);
  ...
  return (
    <main className="min-h-screen bg-white px-6 py-10">
      ...
      <div className="mx-auto max-w-6xl space-y-8">
          ...
          <div className="mt-6 grid gap-8 lg:grid-cols-[1.4fr_0.8fr] lg:items-end">
            <div>
              ...
              <h1 className="mb-4 max-w-3xl text-4xl font-bold text-zinc-900">
                {isDutch
                  ? `Fietshulp per buurt en bekende plek in ${siteConfig.city}`
                  : `Bike help by neighborhood and landmark in ${siteConfig.city}`}
              </h1>
              <p className="max-w-3xl text-lg text-zinc-700">
                {isDutch
                  ? `${siteConfig.siteName} helpt fietsers uit wijken en drukke plekken in ${siteConfig.city} met reparatie, tweedehands fietsen, sloten, verlichting en praktische lokale fietshulp.`
                  : `${siteConfig.siteName} helps cyclists across ${siteConfig.city} neighborhoods and busy landmarks with repairs, second-hand bikes, locks, lights, and practical local cycling support.`}
              </p>
            </div>
            ...
            <div className="rounded-2xl bg-zinc-50 p-6">
              ...
              <p className="font-medium text-zinc-900">{businessName}</p>
          ...
          <section>
            <div className="mb-5">
              <p className="text-sm uppercase tracking-wide text-zinc-500">
                {isDutch ? "Drukke plekken" : "Busy spots"}
              </p>
              <h2 className="mt-2 text-2xl font-bold text-zinc-900">
                {isDutch ? "Bekende plekken" : "Known city spots"}
              </h2>
            </div>
        ...
        {residentialPages.length > 0 ? (
          <section>
            <div className="mb-5">
              <p className="text-sm uppercase tracking-wide text-zinc-500">
                {isDutch ? "Buurten" : "Neighborhoods"}
              </p>
              <h2 className="mt-2 text-2xl font-bold text-zinc-900">
                {isDutch ? "Buurtpagina's" : "Neighborhood pages"}
              </h2>
            </div>
        ...
        <section className="rounded-2xl border border-zinc-200 bg-zinc-50 p-8 text-center shadow-sm">
          ...
          <p className="mx-auto mb-6 max-w-2xl text-zinc-700">
            {renter
              ? isDutch
                ? `Bel ${businessName} of open de route voor snelle hulp met reparatie, aankoopadvies of accessoires voor dagelijks fietsen in ${siteConfig.city}.`
                : `Call ${businessName} or open directions for quick help with repairs, buying advice, or accessories for everyday cycling in ${siteConfig.city}.`
              : isDutch
                ? `Verstuur een fietshulpaanvraag voor ${siteConfig.city} met je fietsprobleem, gewenste fietstype of dagelijkse route.`
                : `Send a bike help request for ${siteConfig.city} with your bike problem, preferred bike type, or daily route.`}
          </p>
